...
当导入数据包含**关联字段**列时，可以为该列指定一个「匹配字段」（通常为关联表的主字段）。导入时系统会按 Excel 列的值在目标表中查找对应记录并自动建立关联，并在导入结果中展示关联匹配统计（已关联 / 未匹配 / 重复匹配）。

#### 成员姓名解析

当导入数据包含**成员字段**列时，系统支持按姓名解析用户：导入时根据列中的姓名自动匹配对应用户 ID 并建立成员关联，并在导入结果中展示成员匹配统计（已匹配 / 未匹配）。
