...
1. 升级后首次启动会自动执行数据库迁移（Alembic），已有数据不会丢失。
2. 升级前仍建议备份 `data/`（数据库）、`uploads/`（附件）与 `config/.env`（配置）。
3. Docker 部署升级镜像时保留 `smarttable_data`、`smarttable_uploads`、`smarttable_redis` 等数据卷即可保留数据。
4. v1.6.6 起 PostgreSQL 连接驱动切换为 psycopg，`DATABASE_URL` 需使用 `postgresql+psycopg://` 前缀。
...
### 编辑者（EDITOR）无法创建字段、工作流或仪表盘

**现象**：编辑者角色在表格中找不到字段管理、工作流等管理入口，或操作时提示无权限。

**解决方案**：

1. v1.6.4 起，字段类与管理类操作（字段管理、工作流、数据表、文档、仪表盘等）的权限已统一收敛至管理员（ADMIN）及以上，避免普通用户误操作。
2. 如需执行此类操作，请让 Base 所有者在成员管理中将该用户角色调整为管理员，或由管理员代为操作。
...
**现象**：多人同时提交同一个分享表单，部分用户收到请求过于频繁的提示。

**解决方案**：

1. v1.6.4 起，分享表单的速率限制已由客户端 IP 维度改为分享令牌维度（每个表单 100 次 / 15 分钟），局域网多用户场景不会再互相影响。
2. 若仍触发限流（如自动化脚本高频提交），请稍后重试，或改用开放 API 方式接入数据。
...
1. 检查附件字段的「文件数量限制」和「文件大小限制」；服务端单文件上限为 50MB。
...
**现象**：字段类型中找不到地理位置字段，或地图选点组件显示空白。

**解决方案**：

1. 地理位置字段（v1.6.6 新增）基于天地图服务，需在服务端环境变量中配置 `TIANDITU_KEY`（在 https://console.tianditu.gov.cn 申请）。
2. 未配置 `TIANDITU_KEY` 时，前端会自动隐藏地图相关入口；配置后刷新页面即可启用。
3. 私有化部署或需要切换镜像时，可通过 `TIANDITU_API_BASE` 替换天地图服务地址。
...
**现象**：工作流 Python 脚本节点（v1.6.4 新增）执行报错，或返回结果为空。

**解决方案**：

1. 脚本在服务端沙箱中以子进程隔离执行，运行环境受限，请优先使用节点配置面板中的「测试运行」验证脚本逻辑。
2. 脚本返回结果有大小限制（约 1MB），避免在脚本中构造或返回超大对象。
3. 检查 Python 语法与变量引用，查看「执行日志」中的报错信息定位问题。
...
**现象**：协作邀请、审批等场景下没有收到任何通知。

**解决方案**：

1. v1.6.4 起通知优先通过站内信推送（页面右上角的铃铛图标），请先检查站内信。
2. 站内信正常但邮件未收到时，检查管理后台「系统设置」中的 SMTP 配置，并使用测试发送功能验证。
3. v1.6.6 起邮件通知中的链接使用「平台入口域名」配置生成，若链接指向错误地址，请在系统设置中配置该项。
...
1. v1.6.5 起支持中英文双语界面，可在页面右上角的语言切换组件中手动切换。
2. 未手动选择时，系统会自动检测浏览器语言并使用对应语言展示。

### 如何开发或安装插件

**解决方案**：

1. v1.6.6 引入插件扩展体系，支持通过插件扩展表格功能，项目内置了前后端示例插件（hello-panel）。
2. 开发方法请参考开发者文档的[插件架构](/zh-CN/developer/plugins/architecture)与[插件开发指南](/zh-CN/developer/plugins/developer-guide)。
