...
本次更新聚焦 **主从表与树形层级能力**、**站内信通知体系**、**工作流自定义脚本节点**、**协同编辑与并发控制** 以及 **权限体系收敛**。
...
- ⭐ **自定义脚本节点**：新增工作流自定义脚本节点，支持 Python 语言的自定义脚本
...
| 问题描述 | 修复内容 |
| --- | --- |
| 下拉选择编辑器 | 修复选中值匹配异常，单选/多选改为按 id 匹配并兼容旧的 name 匹配方式 |
| 下拉选项显示 | 修复显示文本错误，改用实际 name 作为展示内容 |
| 视图路由校验 | 修复 table_id 类型不匹配导致的校验失败，表单视图现可设置为默认视图 |
| 层级表格展开 | 修复展开按钮不显示的问题（CachedDataSource 模式下初始化 records 为空） |
| 表格滚动定位 | 修复数据更新后表格跳回首行的问题 |
| 分享接口路由 | 统一前后端分享接口路由路径，合并更新与删除接口并优化错误提示 |
| 分享接口兼容性 | 修复使用 `filter_by` 替代 `query.get` 以兼容 CompatUUID 类型 |
| 时间戳适配 | 修复后端返回秒级时间戳的前端适配问题 |
| 表单错误提示 | 修复提交错误提示取值顺序，优先读取 error 字段以兼容后端错误格式 |
| 调试重载 | 修复打包模式下 Flask 调试重载导致重复打开浏览器的问题 |
| 公式日期比较 | 修复日期/时间字段值在公式比较中的类型不一致问题 |
| 单元格选项存储 | 修复单元格中调整单选与多选的值，存储的选项值与弹窗里的不一致问题 |
| 表单分享复制 | 修复表单分享链接部分场景下无法复制问题 |
...
- 所见即所得的 HTML 编辑器
- 支持变量插值（`{{username}}`、`{{verification_url}}` 等 12 种变量）
- 实时预览渲染效果
